...
@app.route("/val_docs", methods=["POST"])
@jwt_authenticated
...
def val_tit() -> Response:
    return valida(request, "tit")
        
@app.route("/", methods=["POST"])
@jwt_authenticated
def valida_geral(request):
    operacao = request.form.get("operacao", "doc")
    return valida(request, operacao)

def valida(request, operacao):
    try:
        prompt = getPrompt(request, operacao)
        documentParts = getDocumentsParts(request)
        #logger.info(f"Prompt: {prompt} - Docs: {len(documentParts)}")
        print(f"Prompt: {prompt} - Docs: {len(documentParts)}")
        parts = []
        parts.append(types.Part.from_text(text=prompt))
        parts.extend(documentParts)
        response = getResponse(getConfig(request, operacao), parts)
        if not response:
            return returnError()
        return Response(
                status=200,
...
def getConfig(request, operacao):
    if operacao == "doc":
        return valida_documentos_config
    elif operacao == "tit":
        return valida_titular_config
    return genai_util.getGenAiConfig(request.form.get("prompt"))
...
def getDadosValidacao(request):
    return request.form.get("nome_titular"), int(request.form.get("quantidade_dependentes",0))
